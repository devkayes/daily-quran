...
from conventional commits on `main` — nobody bumps `version` or edits
`CHANGELOG.md` by hand.

1. Merging a conventional commit (`feat:`, `fix:`, ...) to `main` makes
   release-please open or update a "release PR" that bumps `package.json`,
   `.release-please-manifest.json` and `CHANGELOG.md`.
2. Merging that PR tags the release and triggers `.github/workflows/release.yml`,
   which builds `pnpm zip` and `pnpm zip:firefox` and uploads them as workflow
   artifacts.
3. Store submission in the same job is opt-in: it only runs once the
   `CHROME_*`, `FIREFOX_*` and `EDGE_*` secrets referenced in that workflow
   exist as repository secrets. Until then, download the artifacts and upload
   them by hand to the [Chrome Web Store](https://chrome.google.com/webstore/devconsole),
   [Firefox Add-ons](https://addons.mozilla.org/developers/) and the
